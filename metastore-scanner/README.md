...
| **`find_roots`** | Отдельные корни для режима `find_names` (если не пусто — используются вместо `scan_paths` для поиска). |
...
| **`--find-discover-all`** | Вместе с **`--find`**: искать **все** совпадения на диске под корнем. **Без** этого флага по умолчанию обрабатываются только файлы, **уже есть в локальной базе** под шаблоном. |
...
py -m scanner.cli scan --find "*.pdf" --find-discover-all --path "C:\Users\you\Downloads"
